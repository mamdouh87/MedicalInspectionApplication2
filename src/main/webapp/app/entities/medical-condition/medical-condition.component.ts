...
export class MedicalConditionComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.medicalConditionService
      ...
      .subscribe(
        (res: HttpResponse<IMedicalCondition[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInMedicalConditions(): void {
    this.eventSubscriber = this.eventManager.subscribe('medicalConditionListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IMedicalCondition[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/medical-condition'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
